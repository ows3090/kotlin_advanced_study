package ows.kotlinstudy.sns_application.mypage
...
import ows.kotlinstudy.sns_application.databinding.FragmentMypageBinding
                    ...
                    auth.signInWithEmailAndPassword(email, password)
                        .addOnCompleteListener(requireActivity()) { task ->
                            if(task.isSuccessful){
                                ...
                                Toast.makeText(context, "로그인에 실패했습니다. 이메일 또는 비밀번호를 확인해주세요.", Toast.LENGTH_LONG).show()
                            }
        ...
        fragmentMypageBinding.signUpButton.setOnClickListener {
            binding?.let { binding ->
                ...
                auth.createUserWithEmailAndPassword(email,password)
                    .addOnCompleteListener(requireActivity()) { task ->
                        if(task.isSuccessful){
                            Toast.makeText(context, "회원가입에 성공했습니다. 로그인 버튼을 눌러주세요", Toast.LENGTH_LONG).show()
                        } else{
                            Toast.makeText(context, "회원가입에 실패했습니다. 이미 가입한 이메일일 수 있습니다.", Toast.LENGTH_LONG).show()
                        }
                    }
            }
        }
